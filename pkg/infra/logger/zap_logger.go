...
func NewApiLogger(conf *config.Config) *apiLogger {
	return &apiLogger{conf: conf}
}
...
func (l *apiLogger) getLoggerLevel(conf *config.Config) zapcore.Level {
	level, exist := loggerLevelMap[conf.Logger.ZapLogger.Level]
	if !exist {
...
func (l *apiLogger) InitLogger() {
	logLevel := l.getLoggerLevel(l.conf)

	logWriter := zapcore.AddSync(os.Stderr)

	var encoderCfg zapcore.EncoderConfig
	if l.conf.App.Env == "Development" {
	...
	if l.conf.Logger.ZapLogger.Encoding == "console" {
